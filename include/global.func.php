...
//激活码的验证
function _active_yzm($f_yzm,$s_yzm){
	...
	if($type==1){
		...
		echo '</div>';
			...
			echo '<ul>';
					...
					echo '<li>共有<strong> '.$num.'</strong>神器	</li>';
					 if($page==1)
					{	echo '<li>首页		|</li>';
						echo '<li>上一页		|</li>';
					}else{
						echo '<li><a href="'.$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'].$id.'&page=1">首页		|</a></li>';
						//不是首页就可以上一页
						echo '<li><a href="'.$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'].$id.'&page='.($page-1).'">上一页		|</a></li>';
					...
					if($page==$pageabsolute){
						...
						//不是尾页就可以下一页
						echo '<li><a href="'.$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'].$id.'&page='.($page+1).'">下一页		|</a></li>';
						echo '<li><a href="'.$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'].$id.'&page='.($pageabsolute).'">尾页		|</a></li>';
					}
					echo '</ul>';
			echo '</div>';
	}
